...
cont5		equ		h'4D'
	...
	;------------->;Nota 3: Esta implementación es por fuerza bruta. No es la manera más elegante de hacerlo, pero funciona y está fácil de entender.
		...
		movlw	d'65'	;Iniciarlos con 5 veces el valor de los segundos que requiramos debido a la frecuencia del oscilador
		...
		movlw	d'25'	;En el caso del contador de 25 segundos, el valor será de 25
		movwf	cont5
...
c1: 	movlw	b'00100001'		;Al comenzar el primer loop, ponemos la primera secuencia en el PORTB
		...
		movlw	d'25'
		movwf	cont5
loopb:	movlw 	d'20'			;Es importante mencionar que si queremos la versión que correrá sobre el PIC, es necesario
...
c2: 	movlw	b'00100010'		;Comenzamos el segundo loop y metemos el siguiente valor de la secuencia en el PORTB
		movwf	PORTB
lb2:	movlw 	d'20'			;Luego volvemos a inicializar los contadores
		movwf	cont1
lc2: 	movwf 	cont2
ld2:	movwf	cont3
le2:	decfsz	cont3,f
...
c3: 	movlw	b'00001100'		;por lo cual ponemos el valor de la secuencia 3 en el PORTB
	...
	;En este punto tenemos que reiniciar los contadores de 5 y 25 segundos
		...
		movlw	d'25'
		movwf	cont5
lb3:	movlw 	d'20'	;Reiniciamos los contadores de los loops anidados. Recordar que la versión de prueba del programa trabaja con 		
...
c4: 	movlw	b'00010100'		
		movwf	PORTB
lb4:	movlw 	d'20'			;Luego volvemos a inicializar los contadores
		movwf	cont1
lc4: 	movwf 	cont2
ld4:	movwf	cont3
le4:	decfsz	cont3,f
